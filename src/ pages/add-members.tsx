...
function AddMembers() {
  ...
  const [addMemberstMutation, { data: addMembersResult, loading }] = useMutation<
    addMembers,
    addMembersVariables
  >(NEW_MEMBERS_MUTATION, {
    onCompleted,
  });
  ...
  const moveLeader = () => {
    if (selectedListValues !== null) {
      ...
      selectedListValues.map(
        (item) => (leaderList = leaderList.filter((x) => x.value !== item.value))
      );
  ...
  const moveLeft = () => {
    if (selectedListValues !== null) {
      ...
      selectedListValues.map(
        (item) => (optionsList = optionsList.filter((x) => x.value !== item.value))
      );
  ...
  const handleChange = (e:ChangeEvent<HTMLSelectElement>) => {
    ...
    const temp = Array.from(e.target.selectedOptions, (option) => new Object({"value":option.value,"label":option.label}));
  ...
  useEffect(() => {
  }, [firstListValues, secondListValues,selectedListValues]);
      ...
      <Helmet>
        ...
        <div className="flex justify-center items-center">
          ...
          <div className="mx-5 flex justify-center items-center">
            <div className="">
              <div className="p-3">
              <button className="hover:bg-gray-400 transition w-16 h-10 bg-mainBlue text-white rounded-md" onClick={() => moveLeader()}>{"PL"}</button>
              </div>
              <div className="p-3">
              <button className="hover:bg-gray-400 transition w-16 h-10 bg-mainBlue text-white rounded-md" onClick={() => moveAllRight()}>{">>"}</button>
              </div>
              <div className="p-3">
              <button className="hover:bg-gray-400 transition w-16 h-10 bg-mainBlue text-white rounded-md" onClick={() => moveRight()}>{">"}</button>
              </div>
              <div className="p-3">
              <button className="hover:bg-gray-400 transition w-16 h-10 bg-mainBlue text-white rounded-md" onClick={() => moveAllLeft()}>{"<<"}</button>
              </div>
              <div className="p-3">
              <button className="hover:bg-gray-400 transition w-16 h-10 bg-mainBlue text-white rounded-md" onClick={() => moveLeft()}>{"<"}</button>
              </div>
            </div>
          </div>
          <div className="flex flex-col">
            <div className="flex justify-center items-center bg-mainBlue h-12 text-white">프로젝트 리더</div>
            <select className="w-64 rounded-md p-4 h-16 shadow-xl outline-0 outline-mainBlue"
            ...
            <div className="flex justify-center items-center bg-mainBlue h-12 text-white">프로젝트 멤버</div>
            <select
        ...
        <div className="my-5">
          <button type={"button"} className="rounded-lg border-2 border-mainBlue px-4 py-2 w-40 text-mainBlue mx-1 hover:bg-mainBlue hover:text-white transition" onClick={onSubmit}>완료</button>
        </div>
      </div>
  );
}
