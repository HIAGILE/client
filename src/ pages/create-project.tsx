...
export function CreateProject() {
  ...
  const [createProjectMutation, { data: createProjectResult, loading }] =
    useMutation<createProject, createProjectVariables>(PROJECT_MUTATION, {
      onCompleted,
    });
  ...
  const selectedAgile = [
    {
      name: '스크럼',
      code: 'SCRUM',
      description: '스크럼으로 시작하기',
      tag: ['#스크럼', '#스프린트', '#생명주기'],
    },
    {
      name: 'EX',
      code: 'EX',
      description: '익스트림 프로그래밍으로 시작하기',
      tag: ['#익스트림 프로그래밍', '#빠르고 날쌘'],
    },
    {
      name: '짝 프로그래밍',
      code: 'PAIR',
      description: '짝 프로그래밍으로 시작하기',
      tag: ['#짝 프로그래밍'],
    },
    {
      name: '칸반보드',
      code: 'KANBAN',
      description: '칸반보드로 시작하기',
      tag: ['#칸반보드', '#실용적인'],
    },
  ];
  ...
  return (
    <div className="h-full px-10 pt-28 rounded-3xl bg-white">
      ...
      <p className="font-bold text-3xl h-16">
        Create New Project{agileCode ? `(${agileCode})` : ''}
      </p>
        ...
        {!agileCode &&
          selectedAgile.map((agile, index) =>
            index === 0 ? (
              <div
                key={agile.code}
                className="max-w-xs m-3 hover:scale-105 hover:bg-gray-100 transition duration-300 ease-in-ou rounded-md overflow-y-hidden shadow-lg"
                onClick={() => {
              ...
              >
                <img
                  className="w-full"
                  src="https://imagedelivery.net/6qzLODAqs2g1LZbVYqtuQw/9f5d38d5-0fb4-4dfb-f6e3-a39397620700/public"
                  alt="Sunset in the mountains"
                ></img>
                <div className="px-6 py-4">
                  <div className="font-bold text-xl mb-2">{agile.name}</div>
                  <p className="text-gray-700 text-base">{agile.description}</p>
                </div>
      ...
      {agileCode && (
        ...
        >
          {!next && (
            <>
              <p className="p-8 text-xl font-bold text-black">Project Name*</p>
              <input
                {...register('name', {
              ...
              <p className="p-8 text-xl font-bold text-black">Github URL</p>
              <input
                {...register('githubURL', {
              ...
              <button
                type={'button'}
                className="rounded-lg border-2 border-mainBlue px-4 py-2 w-40 text-mainBlue mx-1 hover:bg-mainBlue hover:text-white transition duration-300 ease-in-out"
                onClick={goBackStep}
              >
                이전
              </button>
